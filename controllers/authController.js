...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    return next(
      new AppError('you are not logged in! Please log to get access.', 401)
    );
  }
  ...
  if (!freshUser) {
    return next(
      new AppError('the user belonging to this token does no exists.', 401)
    );
  }
  ...
  if (freshUser.changedPasswordAfter(decoded.iat)) {
    return next(new AppError('Passwod changed!Please login again'), 401);
  }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  const message = `Forgot your password ? Submit a PATCH request with your new password and passwordConfirm to: ${resetURL}. \n If you didn't your password plese forgot your email.`;
try{
